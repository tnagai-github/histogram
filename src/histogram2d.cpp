...
int main(int argc, char *argv[])
{
  ...
  opt.add_options()
    ...
    ("bGnuplotPm3d"   ,  po::value<bool>()->default_value(true),     "if true, a blank line is added after each 'block' in output file, \nso that the pm3d option in gnuplot works.")
  ...
  try{
    ...
    else
    {
      ...
      bGnuplotPm3d = vm["bGnuplotPm3d"].as<bool>();
  ...
  cout << "bGnuplotPm3d: " << bGnuplotPm3d << endl;
  ...
  const int  nbins[DIM_HIST] = {(int)std::ceil((max[0]-min[0])/dx[0]), (int)std::ceil((max[1]-min[1])/dx[1])};
  vector<vector<double> >  edges;
  vector< vector<int> >    counts(nbins[0],vector<int>(nbins[1],0));
  ...
  {
    auto bsscanf = std::sscanf(buf.c_str(), "%s %s" , tmp[0],tmp[1] );
    if(bsscanf != DIM_HIST){
    ...
    for(int i=0; i<DIM_HIST; i++){
      ...
      if(*err[i]!='\0'){
        cout << "error to convert: " << tmp[i] << "\nExit!!" <<endl;
        cerr << "error to convert: " << tmp[i] << "\nExit!!" <<endl;
        return -1;
      }
    }
  ...
  for(int i=0; i < nbins[0]; i++){
    for(int j=0; j < nbins[1]; j++){
      pdf[i][j] = counts[i][j]/double(ncounts)/(dx[0]*dx[1]);
      free_energy[i][j] = -kB_kJ_per_mol*T*std::log(pdf[i][j]);
    }
  }

  double min_free_energy = free_energy[0][0];
  for(const auto  &v1: free_energy){
    for(const auto &v2: v1){
      min_free_energy=std::min(min_free_energy, v2);
    }
  } 
  ...
  for(int i= 0; i<nbins[0];i++){
    for(int j= 0; j<nbins[1];j++){
      ofs_hist << boost::format("%10.5f %10.5f %12.6g %12d\n") % (edges[0][i]+0.5*dx[0]) % (edges[1][j]+0.5*dx[1]) % pdf[i][j]  %counts[i][j] ;
      ofs_FE   << boost::format("%10.5f %10.5f %12.6g\n")      % (edges[0][i]+0.5*dx[0]) % (edges[1][j]+0.5*dx[1]) % free_energy[i][j] ;
    }
    if(bGnuplotPm3d){
      ofs_hist << endl;
      ofs_FE <<   endl;
